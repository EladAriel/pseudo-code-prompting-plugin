...
# Extract the user prompt using POSIX-compatible method
...
CWD="$(pwd)"
CWD_EXTRACTED=$(echo "$INPUT" | sed -n 's/.*"cwd"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')
if [ -n "$CWD_EXTRACTED" ]; then
  CWD="$CWD_EXTRACTED"
fi
...
case "$PROMPT" in
  *implement*|*create*|*add*|*refactor*|*build*|*generate*|*setup*|*initialize*)
