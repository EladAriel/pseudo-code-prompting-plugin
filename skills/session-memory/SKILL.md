...
The requirement-structurer agent follows the 6-step transformation pipeline. Memory integrates at each step:

#### Step 1: Context Detection
- **Load**: activeContext.md to detect project path changes
- **Check**: Is this same project as last session? (auto-reset if project switched)
- **Action**: Use Bash to detect tech stack (package.json, pyproject.toml, go.mod, etc.)
- **Update**: Record detected tech stack in activeContext.md

#### Step 2: Auto-Compression
- **Load**: activeContext.md (user verbosity preference, compression style)
- **Check**: User preference for compression ratio? (stored in past sessions)
- **Action**: Compress requirement if >1000 chars, apply learned compression style
- **Update**: Record compression ratio achieved in progress.md

#### Step 3: Transform to Pseudo-Code
- **Load**: activeContext.md (naming conventions), patterns.md (transformation patterns)
- **Check**: User's preferred naming style (snake_case, camelCase)? Tech stack naming conventions?
- **Apply**: Use learned naming patterns from patterns.md section "User-Specific Conventions"
- **Action**: Apply PROMPTCONVERTER format with user-preferred conventions
- **Update**: Record new transformation pattern discovered in patterns.md if this is a novel requirement

#### Step 4: Validation (Embedded)
- **Load**: patterns.md (validation patterns), progress.md (recurring validation failures)
- **Check**: What validation failures have occurred before? (to catch proactively)
- **Action**: Apply learned validations to catch issues before optimization
- **Update**: Record validation patterns and recurring issues

#### Step 5: Optimization
- **Load**: patterns.md (successful optimizations, security patterns), progress.md (optimization results)
- **Check**: What parameters have worked before? What security requirements are standard?
- **Action**: Apply optimization patterns discovered from previous transformations
- **Update**: Record optimization success in progress.md

#### Step 6: Bridge Offer & Update All Memory
- **Load**: All 3 files to prepare final update
- **Action**: If user chooses bridge to cc10x, conversion happens automatically
- **Update**:
  - activeContext.md: Record completed transformation, update timestamp
  - patterns.md: Add new patterns discovered during this transformation
  - progress.md: Update metrics (compression, validation result, optimization effectiveness)

#### End of Pipeline
After requirement-structurer completes, ALL 3 memory files are updated:
- **activeContext.md**: Current transformation completed, learnings recorded
- **patterns.md**: New patterns discovered or confirmed
- **progress.md**: Quality metrics and transformation history updated

### requirement-validator (2-Step Process)

The requirement-validator agent validates pseudo-code against learned patterns:

#### Step 1: Load Optional Context
- **Load**: activeContext.md (if available - provides project context)
- **Check**: What domain/project is this validation for? (helps identify relevant patterns)
- **Action**: Use context to select appropriate validation patterns for domain

#### Step 2: Comprehensive Validation with Memory
- **Load**: patterns.md (domain-specific patterns), progress.md (learned validation rules)
- **Check**: What patterns apply to this pseudo-code? What issues have occurred before?
- **Action**: Apply learned validation patterns from previous sessions:
  - Security patterns from patterns.md
  - Recurring issue detection from progress.md (e.g., "Rate limiting often missed on endpoints")
  - Tech stack specific patterns from patterns.md
- **Proactive Detection**: If progress.md shows "Missing audit_log caused failures before", proactively check for it
- **Update**:
  - patterns.md: Record validation patterns discovered
  - progress.md: Update validation history and pass rates
  - activeContext.md: Record validation learnings if new insights gained

#### End of Validation
After requirement-validator completes:
- **activeContext.md**: Updated with validation learnings if critical
- **patterns.md**: Updated with new validation patterns discovered
- **progress.md**: Updated with validation results and pass rate metrics
