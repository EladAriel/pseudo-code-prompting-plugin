...
## V2 COMMAND-SPECIFIC INTEGRATION

### Transform Command with Session Memory

The `/transform` command (powered by requirement-structurer agent) integrates memory across all 6 steps:

#### Load Phase (Command START)
...
# Step 2: Load relevant memory files
Read(file_path=".claude/pseudo-code-prompting/activeContext.md")  # User preferences, naming style
Read(file_path=".claude/pseudo-code-prompting/patterns.md")        # Learned transformation patterns
Read(file_path=".claude/pseudo-code-prompting/progress.md")        # Transformation history, metrics
```

#### Transformation Phase (During 6 Steps)

**Concrete Example:**
- User input: "Add authentication endpoint with rate limiting"
- Pattern loaded: From patterns.md "Authentication Endpoints" → rate_limit={"max": 5, "window": "1h"}
- Pattern applied: Auto-includes rate limiting in pseudo-code output
- Result: `create_endpoint(..., rate_limit={"max": 5, "window": "1h"}, ...)`

**Step-by-Step Memory Usage:**
1. **Context Detection**: Uses activeContext.md to check for project path changes (auto-reset if needed)
2. **Auto-Compression**: Applies user's compression preference from activeContext.md
3. **Transform**: Applies naming conventions from activeContext.md, patterns from patterns.md
4. **Validate**: Checks validation patterns from progress.md (recurring failures)
5. **Optimize**: Adds security patterns from patterns.md, optimization history from progress.md
6. **Bridge**: Offers cc10x conversion with full context

#### Update Phase (Command END)

```python
# After transformation completes, update all 3 files

# Step 1: Read current state (before editing)
Read(file_path=".claude/pseudo-code-prompting/activeContext.md")

# Step 2: Update activeContext - record what was transformed + preferences learned
Edit(file_path=".claude/pseudo-code-prompting/activeContext.md",
     old_string="## Recent Transformations",
     new_string="## Recent Transformations
- Authentication Endpoint: Input → `create_endpoint(...)` (compression: 92%, validation: pass)
- [Previous transformation 1]
- [Previous transformation 2]
[Keep last 10 only]")

# Step 3: Update patterns - record new patterns discovered
Read(file_path=".claude/pseudo-code-prompting/patterns.md")
Edit(file_path=".claude/pseudo-code-prompting/patterns.md",
     old_string="## REST API Patterns",
     new_string="## REST API Patterns

### Authentication Endpoints (Confirmed Session X)
```javascript
create_endpoint(
...
# Step 4: Update progress - record metrics
Read(file_path=".claude/pseudo-code-prompting/progress.md")
Edit(file_path=".claude/pseudo-code-prompting/progress.md",
     old_string="## Transformation History",
     new_string="## Transformation History
- [x] Authentication endpoint - 92% compression, validation pass, rate_limit pattern applied
[Keep last 20 only]")

# Step 5: Update timestamp in all files
Edit(file_path=".claude/pseudo-code-prompting/activeContext.md",
     old_string="## Last Updated",
     new_string="## Last Updated
2026-02-01 14:30:00 - Transform command completed, 1 pattern confirmed")
```

### Validate Command with Session Memory

The `/validate` command (powered by requirement-validator agent) uses memory to reduce recurring issues:
...
# Step 2: Load patterns and progress (primary focus)
Read(file_path=".claude/pseudo-code-prompting/patterns.md")        # Validation patterns learned
Read(file_path=".claude/pseudo-code-prompting/progress.md")        # Validation history, failures
Read(file_path=".claude/pseudo-code-prompting/activeContext.md")   # Project context (optional)
```

#### Validation Phase (Comprehensive Checks with Memory)

**Concrete Example:**
- Input pseudo-code: `create_user_endpoint(path="/api/users", method="POST", schema={...})`
- Memory check: progress.md shows "Missing rate_limit caused 2 critical failures before"
- Proactive detection: CRITICAL issue found before user even mentions it
- Output: "⚠ CRITICAL: No rate limiting - previous transformations failed validation due to this"

**Memory-Enhanced Validation:**
1. Load patterns.md to get domain-specific validation patterns
2. Check progress.md recurring issues section:
   - "Rate limiting often missed on public endpoints" → Check for it proactively
   - "Audit logging needs explicit mention" → Flag if missing
   - "Error response structure incomplete" → Validate structure
3. Use activeContext.md project context to identify relevant patterns
4. Validation checks use learned patterns (5-10% higher catch rate from previous sessions)

#### Update Phase (Command END)

```python
# After validation completes, update memory with new insights

# Step 1: Read current patterns
Read(file_path=".claude/pseudo-code-prompting/patterns.md")

# Step 2: Record validation patterns discovered
Edit(file_path=".claude/pseudo-code-prompting/patterns.md",
     old_string="## Common Gotchas",
     new_string="## Common Gotchas
- Missing rate limiting on public endpoints (Session X: validated 5 endpoints, all needed this)
- Incomplete validation error messages (Session X: 3 cases)
- Implicit vs explicit security requirements (Session X: 4 cases)
[Previous gotchas...]")

# Step 3: Update progress - record validation results
Read(file_path=".claude/pseudo-code-prompting/progress.md")
Edit(file_path=".claude/pseudo-code-prompting/progress.md",
     old_string="## Validation History",
     new_string="## Validation History
- [x] Create user endpoint - Status: BLOCKED, Critical: 3 (auth, error_handling, rate_limit)
- [x] JWT authentication - Status: READY, All checks passed
[Keep last 20 only]")

# Step 4: Update learnings in activeContext
Read(file_path=".claude/pseudo-code-prompting/activeContext.md")
Edit(file_path=".claude/pseudo-code-prompting/activeContext.md",
     old_string="## Learnings This Session",
     new_string="## Learnings This Session
- Rate limiting validation catches 95% of endpoint issues (up from 87% last session)
- Proactive checks (loaded from memory) prevented 2 critical misses today
[Previous learnings...]")

# Step 5: Update all timestamps
Edit(file_path=".claude/pseudo-code-prompting/activeContext.md",
     old_string="## Last Updated",
     new_string="## Last Updated
2026-02-01 14:35:00 - Validate command completed, validation success rate improved to 95%")
```
